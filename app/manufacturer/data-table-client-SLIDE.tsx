 ...
 *  - Imports type from actions
 *  - Uses manufacturerColumns (typed to Manufacturer)
 *  - Row click → opens Sheet with types list (no redirect)
 */
...
import { ManufacturerTypesSheet } from "./manufacturer-types-sheet";
...
export function ManufacturerTableClient({ data }: ManufacturerTableClientProps) {
  ...
  function handleRowClick(manufacturer: Manufacturer) {
  ...
  return (
    <>
      {/* Sheet — renders outside the table DOM but inside the component */}
      <ManufacturerTypesSheet
        open={sheetOpen}
